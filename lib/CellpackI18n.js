    ...
    translations(text, subs) {
        ...
        try {
            ...
            if (this.environment.get('debug')) {
                ...
                return `<blink><div class="microb-error" style="color:red; background-color:#bbb; border:solid 1px #333; border-radius:3px; padding:10px; font-size:1em; font-family:Monospace"><strong>Translation error: </strong>${err}<br><strong>In: </strong>${text}<br><strong>With: </strong>${subs}</div></blink>`;
            }
        }
    }
    request(connection) {
